...
"""
import pytest
from dmjedi.model.datavault import HubEntity, SatelliteEntity, LinkEntity, AbstractSatAttachable
...
def _prepare_invalid_satellite(sat_entity: AbstractSatAttachable = None) -> SatelliteEntity:
    """
    Returns an invalid instance of `SatelliteEntity`.
    The instance does not contain any fields and thefefore is not valid.
    """
    return SatelliteEntity(name="invalidsat1_s", display_name="Invalid Satellite", entity=sat_entity)


def _prepare_valid_satellite(sat_entity: AbstractSatAttachable = None) -> SatelliteEntity:
    """
    Return a valid instance of `SatelliteEntity` which contains one text attribute.
    """
    s = SatelliteEntity(name="sat2_s", display_name="Valid Satellite", entity=sat_entity)
